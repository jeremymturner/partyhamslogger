...
- ✅ **Function-key macros — DONE.** F1–F12 with N1MM-style `{VAR}` substitution
  ...
  *and* the native Flex CWX API, voice `.wav` for phone, editor dialog, and
  **per-event (per-contest) macro preferences** with Field Day defaults modeled on
...
- ✅ **Multiple logs — DONE.** File → New Log / Open Log / Open Recent switches the
  active log at runtime (rebuilds the window for that log's contest); the Open Log
  chooser lists every saved log with its activity, call, QSO count, and
...
- **Phase 1 — MVP (in progress):**
  ...
  - ✅ **Hamlib CAT auto-fill — DONE.** `rigctld` client validated against a fake
    ...
    your FT-891 via `make rig-spike`.
  - ✅ **FlexRadio native Ethernet API — DONE (read path).** SmartSDR client:
    VITA-49 UDP discovery → radio info; TCP control handshake + `sub slice/radio`
    → live slice frequency/mode, current band, and per-band settings; slice
    tune/mode set. Wire parsing in `radio/flex_protocol.py`, client in
