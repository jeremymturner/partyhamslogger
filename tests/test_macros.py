...
from partyhams.app.macros import DEFAULT_WPM, MacroSet, expand, load_macros, save_macros
...
def test_field_day_default_macros():
    ...
    assert set(macros) == {"CW", "PHONE"}
    cw = {m.key: m.content for m in macros["CW"]}
    assert cw[1] == "CQ FD {MYCALL} {MYCALL} FD"
    assert "{LOG}" in cw[3]  # F3 TU logs the QSO
    assert cw[12] == "{WIPE}"
...
def test_load_defaults_and_round_trip(tmp_path):
    ...
    assert ms.get("CW", 1).content == "CQ FD {MYCALL} {MYCALL} FD"

    # Customize + save, then reload from the per-contest file.
    ms.cw_wpm = 32
    ms.get("CW", 1).content = "CQ TEST {MYCALL}"
    ...
    assert reloaded.get("CW", 1).content == "CQ TEST {MYCALL}"
